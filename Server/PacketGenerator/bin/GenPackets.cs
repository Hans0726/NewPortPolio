...
public class C_PlayerDeckInfo : IPacket
{
    ...
    public void Deserialize(ArraySegment<byte> segment)
    {
        ...
        this.cardss.Clear();
		ushort cardsLen = BitConverter.ToUInt16(s.Slice(count));
		count += sizeof(ushort);
		for (int i = 0; i < cardsLen; i++)
		{
		    Cards cards = new Cards();
		    cards.Deserialize(s, ref count);
		    cardss.Add(cards);
		}
    ...
    public ArraySegment<byte> Serialize()
    {
        ...
        success &= BitConverter.TryWriteBytes(s.Slice(count), (ushort)this.cardss.Count);
		count += sizeof(ushort);
		
		foreach (Cards cards in cardss)
		    success &= cards.Serialize(s, ref count);
