...
=======
- **직접 설계·작성한 기능**
[유저 데이터 패킷 정의](Server/UserData), [유저 데이터 관리](Server/UserData.cs)
매칭 시작부터 게임 시작 후 카드 데이터 설정과 덱 구성(리팩토링으로 인해 작성한 코드가 나뉘었습니다.)
다른 UIPopup 공통 기능을 가지고 있는 [부모 클래스](Assets/Scripts/UIPopup/UIPopup.cs)
모든 UI/UX, 게임맵, 카드 [설계](Assets/Scripts/CardData/CardData.cs) 및 [디자인](Assets/Scripts/CardUI.cs)

- **기존 코드 또는 AI 제안을 직접 수정한 사례**

초기에 어떤 아키텍처나 디자인 패턴을 염두하지 않고 코드를 작성하다 점점 한 클래스에 많은 책임이 들어가게 되거나 클래스 간 데이터 공유가 복잡해지고 가독성이 나빠지는 등의 문제가 생겨
GPT CODEX에게 어떤 구조로 바뀌면 더 나은지 물어보고 [해당 문서](Docs/InGameArchitecture.md)를 보며 [새로운 구조](#씬-초기화와-역할-분리)로 변경하게 되었습니다.
먼저 그 구조에 대해 자세히 알아보고자 직접 로비씬의 카드 덱 편집과 매칭까지 **관련된 모든 스크립트**를 직접 리팩토링하였습니다.
게임씬에 해당하는 부분의 리팩토링은 AI에게 맡겼습니다.

- **직접 확인한 과정** 
[작성 필요: 디버깅 방법, 테스트 조건, 확인 결과]
>>>>>>> 297e852c43d81dabd666056551f2ba38283d891e

### 학습 기반과 AI 활용

서버 프레임워크와 XML 패킷 생성기는 [C# Unity 게임 서버 강의](https://www.inflearn.com/course/%EC%9C%A0%EB%8B%88%ED%8B%B0-mmorpg-%EA%B0%9C%EB%B0%9C-part4?cid=324941#reviews)의 실습에서 출발했습니다.
